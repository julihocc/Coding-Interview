...
def run_tests(solutions, test_cases, runner_func, section_name=None, report_dir=None):
    ...
    # Open report file for writing with UTF-8 encoding
    with open(report_path, "w", encoding="utf-8") as report_file:
        ...
        max_case_id = max(
            len(str(getattr(case, "id", i + 1))) for i, case in enumerate(test_cases)
        )
        ...
        total_width = (
            sol_width + case_width + status_width + time_width + 9
        ...
        # Print header
        write_both(
            f"{'Solution':<{sol_width}} | {'Case':<{case_width}} | {'Status':<{status_width}} | {'Time (μs)':<{time_width}}"
        )
        ...
        for sol_name, sol_func in solutions:
            for i, case in enumerate(test_cases):
                ...
                try:
                    ...
                    write_both(
                        f"{sol_name:<{sol_width}} | {case_id:<{case_width}} | {status:<{status_width}} | {duration_us:<{time_width}.1f}"
                    )
                except Exception as e:
                    write_both(
                        f"{sol_name:<{sol_width}} | {case_id:<{case_width}} | ERROR      | 0.0"
                    )
                    error_msg = (
...
def run_judge_from_file(
    judge_file_path, test_cases, run_case_logic, section_name="SOLUTIONS"
):
    """
