"""Problem2 - MedianMaintainingHeap implementation
...
class MinHeap:
    """Internal MinHeap implementation."""
...
class MaxHeap:
    """Internal MaxHeap implementation."""
...
class MedianMaintainingHeap:
    ...
    Maintains invariant that max(hmax) <= min(hmin) and sizes differ by at most 1.
    ...
    def balance_heap_sizes(self):
        """Ensure hmax.size() == hmin.size() or hmax.size() + 1 == hmin.size()."""
        if self.hmax.size() > self.hmin.size():
            mv = self.hmax.max_element()
            self.hmax.delete_max()
            self.hmin.insert(mv)
            return
        if self.hmin.size() > self.hmax.size() + 1:
            mv = self.hmin.min_element()
    ...
    def insert(self, elt):
        ...
        if self.hmax.size() == 0:
            if elt > self.hmin.min_element():
                cur = self.hmin.min_element()
                self.hmin.delete_min()
                self.hmin.insert(elt)
                self.hmax.insert(cur)
            else:
    ...
    def get_median(self):
        """Return the median of all elements inserted so far."""
        if self.hmin.size() == 0:
...
def solve():
    """Test runner for MedianMaintainingHeap validation."""
