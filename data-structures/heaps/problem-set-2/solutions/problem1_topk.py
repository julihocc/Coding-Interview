"""Problem1 - TopKHeap tests (per-file)
"""
def solve():
    class MinHeap:
    ...
    class TopKHeap:
        def __init__(self,k): self.k=k; self.A=[]; self.H=MinHeap()
        def size(self): return len(self.A)+self.H.size()
        def insert_into_A(self,elt):
            self.A.append(elt); j=len(self.A)-1
            while j>=1 and self.A[j]<self.A[j-1]: self.A[j],self.A[j-1]=self.A[j-1],self.A[j]; j-=1
        def insert(self,elt):
            if self.size()<self.k: self.insert_into_A(elt); return
            if len(self.A)>0 and elt<self.A[-1]:
                import bisect
                pos=bisect.bisect_left(self.A,elt); self.A.insert(pos,elt); displaced=self.A.pop(); self.H.insert(displaced)
            else: self.H.insert(elt)
        def delete_top_k(self,j):
            assert self.size()>self.k
            del self.A[j]
            mv=self.H.min_element(); self.H.delete_min(); self.A.append(mv)
            idx=len(self.A)-1
            while idx>=1 and self.A[idx]<self.A[idx-1]: self.A[idx],self.A[idx-1]=self.A[idx-1],self.A[idx]; idx-=1
